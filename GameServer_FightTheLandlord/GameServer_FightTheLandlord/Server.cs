...
namespace GameServer_FightTheLandlord
{
    public class Server
    {
        ...
        private void CallBackAccept(IAsyncResult result)
        {
            ...
            try
            {
                ...
                if (!_allNames.Contains(ipname))
                {
                    ...
                    //NetClient client = new NetClient(t_socket, this);
        ...
        private void RecieveMsg(IAsyncResult client_socket)
        {
            ...
            {
                Console.WriteLine(e.ToString());
                Console.WriteLine("玩家异常(" + client.RemoteEndPoint.ToString() + ")离线");
        ...
        {
            //NetClient client = new NetClient(_allClients[ipname], this);
        ...
        public void CloseServer()
        {
            ...
            try
            {
                if (_allNames == null || _allNames.Count <= 0)
                    return;
                //先关闭子连接
                for (int i = 0; i < _allNames.Count; i++)
                {
