...
public class ClientService : EventView
{
    ...
    private void FixedUpdate()
    {
        ...
        if (_client.MessageBox.Count > 0)
        {
            ...
            switch (recData.cmd)
            {
                case RemoteCMD_Const.RegSucess:
                        {
                        HandelRegSucess(recData);
                    }
                    break;
                case RemoteCMD_Const.RegAlready:
                    {
                        HandeRegAlready(recData);
                    }
                    break;
                case RemoteCMD_Const.RegError:
                    {
                        HandeRegError(recData);
                    }
                    break;
                case RemoteCMD_Const.LoginSuccess:
                    {
                        HandelLoginSucess(recData);
                    }
                    break;
                case RemoteCMD_Const.LoginPwdErr:
                    {
                        HandelLoginPwdErr(recData);
                    }
                    break;
                case RemoteCMD_Const.LoginUserErr:
                    {
                        HandelLoginUserErr(recData);
                    }
                    break;
                ...
                case RemoteCMD_Const.MatchWait:
                    {
                        HandleMatchWait(recData);
                    }
                    break;
                ...
                case RemoteCMD_Const.Player3:
                    ...
                    break;
                case RemoteCMD_Const.Player4:
                    {
                        HandlePlayer4(recData);
                    }
                    break;
                ...
                case RemoteCMD_Const.ReturnLobby:                
                    {
                        HandelReturnLobby(recData);
                    }
                    break;
                case RemoteCMD_Const.ReturnRoom:
                    {
                        HandelReturnRoom(recData);
                    }
                    break;
                case RemoteCMD_Const.CreateRoom:
                    {
                        HandelCreateRoom(recData);
                    }
                    break;
                case RemoteCMD_Const.GetRoomList:
                    {
                        HandelGetRoomList(recData);
                    }
                    break;
            }
        }
    ...
    private void HandelLoginSucess(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_Login, recData);
    }

    private void HandelRegSucess(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_Login, recData);        
    }
    ...
    //private void HandleReplay(RemoteCMD_Data recData)
    //{
    //    dispatcher.Dispatch(ServiceConst.Service_Replay, recData);
    //}
    ...
    private void HandleMatchWait(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_MatchWait,recData);
    }
    ...
    private void HandlePlayer4(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_Player4, recData);
    }
    ...
    private void HandelLoginUserErr(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_UserErr, recData);
    }
    private void HandelLoginPwdErr(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_UserErr, recData);
    }
    private void HandeRegError(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_UserErr, recData);
    }
    private void HandeRegAlready(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_UserErr, recData);
    }

    private void HandelReturnLobby(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_ReturnLobby, recData);
    }
    private void HandelReturnRoom(RemoteCMD_Data recData)
    {
        dispatcher.Dispatch(ServiceConst.Service_ReturnRoom, recData);
    }
    private void HandelGetRoomList(RemoteCMD_Data recData)
    {
        RoomListPanel roomListPanel = Resources.Load<RoomListPanel>("Panel/RoomListPanel");
        roomListPanel.OnGetRoomListResponse(recData.MatchCounts.ToString() );
    }
    private void HandelCreateRoom(RemoteCMD_Data recData)
    {
        RoomPanel roomPanel = Resources.Load<RoomPanel>("Panel/RoomPanel");
        roomPanel.OnCreateRoomResponse(recData.MatchCounts.ToString() );            
    }

    private void HandelLeaveRoom(RemoteCMD_Data recData)
    {
        RoomPanel roomPanel = Resources.Load<RoomPanel>("Panel/RoomPanel");
        roomPanel.OnLeaveRoomResponse(recData.MatchCounts.ToString());
    }

    private void HandelJoinRoom(RemoteCMD_Data recData)
    {
        RoomPanel roomPanel = Resources.Load<RoomPanel>("Panel/RoomPanel");
        roomPanel.OnJoinRoomResponse(recData.MatchCounts.ToString());
    }
}
//----------------------------------------------//
//通讯过程
...
//4.服务器向确定玩家的起始顺序并发送StartPlayer、Player2、Player3,Player4命令
//5.客户端接收StartPlayer、Player2、Player3,player4命令初始化玩家位置和信息
